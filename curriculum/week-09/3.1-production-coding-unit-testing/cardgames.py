...
class War():
    ...
    def __init__(self):
        ...
        self.handTwo = Hand()
        self.tableau = Deck()
        log.debug('Created handOne, handTwo and tableau instances')
        return
        
    def deal(self):
        while len(self.tableau.cards) > 0:
            self.handOne.add_card(self.tableau.draw_card()) # add one card to handOne
            self.handTwo.add_card(self.tableau.draw_card())
            log.debug('Added to hands, {} left in deck'.format(len(self.tableau.cards)))
        log.info('player 2 has {}'.format(' '.join(str(c) for c in self.handTwo.cards)))
        return
    ...
    def turn(self):
        ...
        if self.cardOne.is_equal(self.cardTwo):
            ...
            if not self.handOne.cards:
                self.message = "player 2 wins"
                return self.message

            if not self.handTwo.cards:
                self.message = "player 1 wins"
                return self.message

            # draw until each player puts 3 in tableau or one is down to last card
            while len(self.handOne.cards) > 1 and len(self.handTwo.cards) > 1 and i < 3:
...
if __name__ == "__main__":
    ...
    print 'p1: {}, p2: {}'.format(p1Wins, p2Wins)